...
<?php 
// for viewing all record na gi count sa table in databse
  $connect = mysqli_connect("localhost","root","","practice");

  $sql = "SELECT count(id) AS total FROM stock_update WHERE quantity < 21 LIMIT 1";
...
<div class="w3-container" style="width: 50%">
  <div class="header " style="background-color: ; padding: 5px; width: 100%; color: gray; letter-spacing: 4px;">
    <h2 style="float: left;"><center>View Medicine Stock</center></h2>
...
<?php
 ...
 $query = "SELECT * FROM stock_update WHERE quantity > 20";
    ...
    <tr class="del<?php echo $id ?>" >
      <td style="float: left;"><center><?php echo $row['medicine']; ?></td>
     ...
     <a class="btn btn-info"  id="a<?php echo $id; ?>" data-content="Click here to Edit Employee" rel="popover" data-original-title="Edit?"  href="edit_medicine.php<?php echo '?id='.$id; ?>"><i class="icon-edit icon-large"></i> Update</a>&nbsp;
...
<?php
  // for viewing in child immunization
 $query = "SELECT * FROM stock_update WHERE quantity < 21 LIMIT 5";
    $result = mysqli_query($connection, $query);  
      while($row = mysqli_fetch_array($result)){ $id=$row['id']; ?>
     <p style="color: red"> Data will display if medicine is less than 20 left in stock!</h> </p>


<table class="w3-table w3-border w3-red"> 

  <tr>
    <td><a href="add_stock.php" style="color: white">Add stock!</a></td>
  </tr>
  <tr>
    <td style="width: 50%; color: white"><b>Medicine name w/ no. of stock left:</b></td>
    <td style="width: 60%; color: white"><b><?php echo $row['medicine']; ?></td>
    <td style="color: white"><b><?php echo $row['quantity']; ?></b></td>
  </tr>
